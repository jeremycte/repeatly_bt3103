					...
					<div class="study-deck-inner">
						<div class="question-rectangle">
							<div class="items-in-rectangle">
								<div
									class="cards-studied-3-cards-studentstats inter-semi-bold-white-28px"
									id="cardsStudied"
								>
									Cards Studied: __ cards
								</div>
								<div
									class="cards-tried-placeholder inter-semi-bold-white-28px"
									id="cardStatsDisplay"
								>
									Placeholder for number of tries taken for
									each card
								</div>
								<div
									class="cards-left-8-cards-studentstats inter-semi-bold-white-28px"
									id="cardsUnattempted"
								>
									Cards Left Unattempted: __ cards
								</div>
							</div>
						</div>
					</div>
					<div
						class="overlap-group-txtareaStudyDeck border-1px-quick-silver"
					>
						<div
...
<script>
		...
		try {
			...
			for (let i = 0; i < cardsArray.length; i++) {
				...
				if (currentCard.tries > 1) {
					const improvement =
						", Mastery level " +
						String(cardBoxMap.get(currentCardId)) +
						"-> " +
						String(currentCardBoxType);
					triesString =
						String(currentCard.tries) +
						" tries" +
						improvement +
						"<br />";
					anyChange = true;
					...
					stringDisplay +=
						"Card " +
						(cardOrder.indexOf(currentCardId) + 1) +
						": " +
						triesString;
					statsOuputArray[cardIndex] = stringDisplay;
				}

				const currentCardUpdateRef = doc(
					db,
					"users",
					userEmail,
					"decks",
				...
				if (isGroupDeck) {
					...
					for (const grpDeck of userGroupDecks) {
						if (grpDeck.id === deckId) {
							for (const grpCard of grpDeck["cards"]) {
								if (grpCard.id === currentCardId) {
									grpCard.data.boxType = currentCardBoxType;
									grpCard.data.firstAnswered =
										currentCardFirstAnswered;
									grpCard.data.isWrong = currentCardIsWrong;
								}
							}
						}
					}
			...
			if (anyChange) {
				...
				document.getElementById("cardsStudied").innerHTML =
					"Cards Studied: " + cardsVisited + " cards";
				document.getElementById("cardsUnattempted").innerHTML =
					"Cards Left Unattempted: " + uncertainCards + " cards";
			} else {
				document.getElementById("cardStatsDisplay").innerHTML =
					"You did not study any cards";
				document.getElementById("cardsStudied").innerHTML = "";
				document.getElementById("cardsUnattempted").innerHTML = "";
			}
			...
			if (isGroupDeck) {
				for (const grpDeck of userGroupDecks) {
					if (grpDeck.id === deckId) {
						grpDeck.data.uncertainCards = uncertainCards;
						grpDeck.data.needsRecapping = needsRecapping;
					}
				}
				await updateDoc(userDocRef, { groupDecks: userGroupDecks });
			}
			// #############################################
		} catch (error) {
	...
	export default {
		...
		mounted() {
			...
			const deckCompleted = JSON.parse(
				sessionStorage.getItem("deckCompleted")
			);
